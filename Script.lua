-- // Steal a Brainrot — Final Script v7 for Xeno
-- // Speed Hack, NoClip (walls), Anti TP Back, ESP, Void Touch (Dropdown), Lock Base, Invisible, AntiHit
...
local function antiTPBack()
    spawn(function()
        while Character and HumanoidRootPart do
            ...
            wait(0.05)
            if HumanoidRootPart and (HumanoidRootPart.Position - pos).Magnitude > 50 then
        ...
        voidtouchConn = RunService.Heartbeat:Connect(function()
            ...
            if voidTarget then
                ...
                if char then
                    ...
                    if hrp then table.insert(enemies, hrp) end
                end
            else
            ...
            for _, root in ipairs(enemies) do
                if root and root.Parent then
                    ...
                    if dist <= 30 then
                        local dir = (root.Position - myPos).Unit
                        if dir.Magnitude < 0.1 then dir = Vector3.new(0, 1, 0) end
                        root.Velocity = dir * 8000 + Vector3.new(0, 3000, 0)
                        root.AssemblyLinearVelocity = dir * 8000 + Vector3.new(0, 3000, 0)
                    end
                end
            end
        end)
...
titleText.Text = "Brainrot Hack v7"
